...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // Map movies into the same shape used by SearchController
        $allMovies = $movies
            ->map(fn (Movie $movie) => $this->mapMovie($movie))
            ->values();

        // Group movies into categories by their first genre
        $categories = $allMovies
            ->groupBy(function ($movie) {
                return $movie['genres'][0] ?? 'Other';
            })
            ->map(function ($movies, $genreName) {
                return [
                    'name'   => $genreName,
        ...
        return view('index', compact('heroBanners', 'allMovies', 'categories'));
    }
    ...
    private function mapMovie(Movie $movie): array
    {
        return [
            ...
            'this_movie_is' => $this->splitCsv($movie->this_movie_is),
